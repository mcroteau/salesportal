...
<% } else { %>
		...
		<% for(int i = 0; i < campaignSalesActionView.getElements().size(); i++) { 
		        ...
		        <tr>
					...
					<%String date = DateUtilities.formatDate(DateUtilities.getCurrentSQLDate(), "MM/dd/yyyy");
					...
					<%if(campaignSalesActionDetails.getSalesActionMandatoryDate() != null &&
			        	...
			        	<td>
			               <input id="dfDateReq_<%=i %>" type="text" style="{background-color: #FF6666}" name="dfSalesActionDate_<%=i %>" size="12" value="<%=date%>" onchange="javascript:void(0);"><a href="javascript:calSalesActionDate_<%=i%>.popup();"><img class="button" src="<%=AppSettings.getAppWebPath()%>graphics/icons/calendar.gif" border="0" alt="Click Here to Pick up the date"></a>
			            </td>
		            <% } else { %>
		            	<td>
		               		<input type="text" name="dfSalesActionDate_<%=i %>" size="12" value="<%=date%>" onchange="javascript:void(0);"><a href="javascript:calSalesActionDate_<%= i%>.popup();"><img class="button" src="<%=AppSettings.getAppWebPath()%>graphics/icons/calendar.gif" border="0" alt="Click Here to Pick up the date"></a>
		            	</td>
		            <% } %>
