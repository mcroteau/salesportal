...
<%=AppSettings.getHeader(request, userProfile, AppSettings.getParm("COMPANY_NAME") + ": All Territory Weekly Calendar", "","", "")%>
...
<script language="JavaScript" src="<%=AppSettings.getAppWebPath()%>scripts/calendarSchedule/weekCalendarSchedule.js?Timestamp=<%=String.valueOf((new java.util.Date()).getTime())%>"></script>
...
<% String[] days = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" }; %>
 ...
 <input type="hidden" name="dfCalTerritoryId" value="<%=Utilities.nullToBlank(request.getParameter("dfCalTerritoryId"))%>" >
 ...
 <table border="0" width="99%" cellspacing="0" cellpadding="3">
 		...
 		<tr>
                <td class="navigationbar1"  align="left" nowrap width="10%">
                	<a href="javascript: displayPrintReport(<%=territoryDetails.getTerritoryId()%>);"><img class="button" src="<%=AppSettings.getAppWebPath()%>graphics/icons/print.gif" alt="Print Record"></a>
                    <a href="javascript: displayPrintReport(<%=territoryDetails.getTerritoryId()%>);">Print Weekly Calendar</a>
                    <a href="javascript: displayPrintReportNote(<%=Utilities.nullToBlank(territoryDetails.getTerritoryId())%>);">Print Weekly Calendar With Notes</a>
        </tr>
</table>
	...
	<tr valign="center">
			<th colspan="2" align="left">
					<a href="javascript: fChangeView('PREV');"><img class="button" src="<%=AppSettings.getAppWebPath()%>graphics/icons/previous.gif" alt="Previous"> Previous Week</a>
			</th>
			<th colspan="3" align="center">
					<h2><%=territoryDetails.getTerritory()%><h2>
			</th>
			<th colspan="2" align="right">				
					<a href="javascript: fChangeView('NEXT');">Next Week <img class="button" src="<%=AppSettings.getAppWebPath()%>graphics/icons/next.gif" alt="Next"></a>
			</th>
				...
				<% for(int j=0; j < dailyProspects.size(); j++) { 
					CalendarScheduleDetails calendarScheduleDetails = (CalendarScheduleDetails)dailyProspects.get(j);
						...
						String body = Utilities.escapeProblemCharacters(
								...
								Utilities.nullToBlank(calendarScheduleDetails.getEmail()) + "<br>" +
								...
								Utilities.nullToBlank(calendarScheduleDetails.getActionNote()) + "<br><br><b>Note:</b>&nbsp;&nbsp;" +
								Utilities.nullToBlank(calendarScheduleDetails.getNote()), "");%>
					<table width="100%">
							<tr><td width=150><a href="javascript:fOnUpdate(<%=calendarScheduleDetails.getProspectId()%>);"><img src="<%=AppSettings.getAppWebPath()%>graphics/icons/edit.gif" 
					...
					<table frame="below">
							<tr><td width=150>
									...
									<%=Utilities.nullToBlank(calendarScheduleDetails.getCustomerCompany())%><br>
									<%=Utilities.nullToBlank(calendarScheduleDetails.getContactName())%><br>
									<%=Utilities.nullToBlank(calendarScheduleDetails.getPhone())%>
								</td>
							</tr>
							<tr>
...
<form name="frmPrintWeeklyFormNote" method="post" action="territory_calendar" target="print_week_calendar_note">
 	<input type="hidden" name="formAction" value="DISPLAY">
 	<input type="hidden" name="calendarType" value="WEEK">
