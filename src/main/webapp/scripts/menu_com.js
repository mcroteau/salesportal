...
function UnLoaded(){
    if(typeof(CloseTimer)!='undefined'&&CloseTimer)clearTimeout(CloseTimer);
    Loaded=0;
    Created=0;
...
function Check(WhichMenu,NoOf){
    ...
    for(i=0;i<NoOf;i++){
        ...
        if(!ArrayLoc[array]){
            WebMasterAlert(1,array);
            return false
        }
        La=ArrayLoc[array][0];
        Li=ArrayLoc[array][1];
        Nof=ArrayLoc[array][2];
        if(i==0){
            if(!ArrayLoc[array][3]){
                WebMasterAlert(2,array);
                return false
            }
            if(!ArrayLoc[array][4]){
                WebMasterAlert(3,array);
                return false
            }
            Hg=ArrayLoc[array][3];
            Wd=ArrayLoc[array][4]
            ...
            if(ArrayLoc[array][3])WebMasterAlert(4,array);
...
function Go(){
    ...
    if(FirstCreate){
        if(FirstLineFrame =="" || !parent.frames[FirstLineFrame]){
            WebMasterAlert(0,FirstLineFrame);
            FirstLineFrame=SecLineFrame
        }
        if(FirstLineFrame =="" || !parent.frames[FirstLineFrame]){
            WebMasterAlert(0,SecLineFrame);
            FirstLineFrame=SecLineFrame=DocTargetFrame
        }
        if(FirstLineFrame =="" || !parent.frames[FirstLineFrame]){
            WebMasterAlert(0,DocTargetFrame);
        ...
        FirstWindowWidth=(ExpYes)?FirstLocation.document.body.clientWidth:FirstLocation.innerWidth;
        FirstWindowHeight=1000000;
        SecWindowWidth=(ExpYes)?SecLocation.document.body.clientWidth:SecLocation.innerWidth;
        SecWindowHeight=1000000;
        if(MenuCentered) CalcLeft();
        if(MenuVerticalCentered) CalcTop();
        if(!DomYes){
            CenterText=(MenuTextCentered)?(NavYes)?"<div align='center' style='z-index:2;'>":"align='center'":"";
...
function CalcLeft(){
    ...
    if(MenuCentered!='left'){
        ...
        if(MenuCentered=='right')StartLeft+=Size;
        else StartLeft+=Size/2
    }
}
function CalcTop(){
    ...
    if(MenuVerticalCentered!='top'){
        ...
        if(MenuVerticalCentered=='bottom')StartTop+=Size;
        else StartTop+=Size/2
    }
}
function PositionMenuStruct(CntnrPntr,Tp,Lt){
    ...
    if (PosRecursLevel==2 && AcrossFrames){
        Tp+=VerCorrect;
        Lt+=HorCorrect
    }
    CntnrStyle.top=Cntnr.OrgTop=Tp;
    CntnrStyle.left=Cntnr.OrgLeft=Lt;
    if (PosRecursLevel==1 && FirstLineHorizontal){
        Hori=1;
        Lefti=CntnrWidth-MmbrWidth-2*BorderWidth;
        Topi=0
    }
    else{
        Hori=Lefti=0;
    ...
    while(Mmbr!=null){
        ...
        if(Mmbr.ChildCntnr){
            if(Hori){
                ...
                SubLt=Lt+Lefti+(1-ChildOverlap)*MmbrWidth+BorderWidth;
                SubTp=Tp+Topi+ChildVerticalOverlap*MmbrHeight
            }
...
function ContainerSetUp(Wdth,Hght,NoOff){
    ...
    if (CreateRecursLevel==1&&FirstLineHorizontal){
        ...
        Hght=(BorderBtwnElmnts)?NoOff*(Hght+BorderWidth)+BorderWidth:NoOff*Hght+2*BorderWidth;
        Wdth+=2*BorderWidth
    }
    if(Nav4){
...
function MemberSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
    ...
    if(MemVal.indexOf('<img')==-1&&eval(WhatMenu+'[2]')){
        S=(CreateRecursLevel==1&&FirstLineHorizontal)?'tridown.gif':GraphicsPath+'tri.gif';
        ...
        if(DomYes){
            ...
            MemVal+="<div style='position:absolute; top:"+T+"; left:"+L+"; width:"+W+"; height:"+H+";visibility:inherit; z-index:2;'><img src='"+S+"'></div>";
...
function NavMemberSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
    ...
    if(this.value.indexOf('<img')==-1 &&eval(WhatMenu+'[2]')){
        ...
        ImgStr=(CreateRecursLevel==1&&FirstLineHorizontal)?"<img src='tridown.gif'>":"<img src='"+GraphicsPath+"tri.gif'>";
...
function CreateMenuStructure(MenuName,NumberOf){
    ...
    if(DomYes){
        ...
        WhichMenu+='c';
        Location.document.body.insertAdjacentHTML("AfterBegin","<div id='"+WhichMenu+"' style='visibility:hidden; position:absolute; z-index:2;'><\/div>");
    ...
    if(Exp4){
        ...
        for(i=1;i<NumberOf+1;i++){
            ...
            MemberContainer.InnerString+="<div id='"+WhichMenu+"' style='position:absolute;z-index:2;'><\/div>"
        }
